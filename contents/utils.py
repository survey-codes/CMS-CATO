...
def get_menu_as_dict(menu, lang):
    ...
    if items:
        for item in items:
            lang_item = item.item_lang.filter(
                    language__abbreviation=lang.language.abbreviation
                ).first()
...
def get_menu_item_as_dict(menu_item, lang):
    ...
    if menu_item.page_url:
        if menu_item.page_url.parent:
            url = "{}/{}".format(
                menu_item.page_url.parent.slug_page,
                menu_item.page_url.slug_page
            )
        else:
            url = menu_item.page_url.slug_page
